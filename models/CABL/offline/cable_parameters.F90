...
!    IF(ASSOCIATED(soiltype_metfile)) DEALLOCATE(soiltype_metfile)
  ...
  !   inPwea         - via cable_param_module
    ...
    ! Hacking to run EucFACE
    PRINT *, 'Original casaflux%Nmindep(1) = ', casaflux%Nmindep(1)
    casaflux%Nmindep(1) = 0.225 / 365.0
    PRINT *, 'but in EucFACE run, it becomes ', casaflux%Nmindep(1)
    PRINT *, 'Original soil order = ', casamet%isorder(1)
    casamet%isorder(1) = 9
    PRINT *, 'but in EucFACE run, it becomes ', casamet%isorder(1)
    ! end hacking
...
!              soil%rs20(landpt(e)%cstart:(landpt(e)%cstart+landpt(e)%nap-1))
         ...
         ELSE
           ...
           WRITE(logn,*) 'Note that LAI could be overwritten if using CASA-CNP.'
         ENDIF
         IF (icycle == 0) THEN
            WRITE(logn, patchfmtr)                                            &
...
!jhan:reviewformatting.spacing of whole module
         IF (icycle >= 1) THEN
            ...
            WRITE(logn,patchfmtr)                                              &
               '          Nitrogen deposition  (mg N/m^2/year): ',             &
               casaflux%Nmindep(landpt(e)%cstart :                             &
               (landpt(e)%cstart + landpt(e)%nap-1) ) * 1000.0
          
            WRITE(logn,patchfmtr)                                              &
               '          Nitrogen fixation    (mg N/m^2/year): ',             &
               casaflux%Nminfix(landpt(e)%cstart :                             &
               (landpt(e)%cstart+landpt(e)%nap-1) ) * 1000.0
            
            WRITE(logn,patchfmtr)                                              &
               '          Phosphorus weathering(mg P/m^2/year): ',             &
               casaflux%Pwea(landpt(e)%cstart :                                &
               (landpt(e)%cstart+landpt(e)%nap-1) ) * 1000.0
            
            WRITE(logn,patchfmtr)                                              &
               '          Phosphorus in dust   (mg P/m^2/year): ',             &
