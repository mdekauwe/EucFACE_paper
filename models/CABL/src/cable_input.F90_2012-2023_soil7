...
!        CALL casa_init(casabiome,casamet,casapool,casabal,veg,phen)
        ...
        PRINT *, 'Starting LAI is now set to: ', casamet%glai(1)
      ENDIF
...
! Hacking to run EucFACE
! put in site specific parameter values
    soil%isoilm(1) = 1
    soil%silt(1)   = 0.09
    soil%clay(1)   = 0.11
    soil%sand(1)   = 0.80
    ...
    soil%hyds(1)   = 6.0e-6
    soil%sucs(1)   = -0.299
    soil%rhosoil(1)= 1700.0
!    veg%hc(1)      = 20.0
    veg%dleaf(1)   = 0.06364
    veg%xfang(1)   = -0.3
    veg%vcmax(1)   = 54.3e-6
    veg%ejmax(1)   = 109.8e-6
    PRINT *, 'Hacking to input EucFACE soil and veg parameters.'
...
! end Hacking to run EucFACE
    ...
    PRINT *, 'Reporting final parameters in log file.'
