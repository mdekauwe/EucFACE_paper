...
real tair_h(1,1,48*(20*365+5)), rh_h(1,1,48*(20*365+5)), wind_h(1,1,48*(20*365+5))
real swdown_h(1,1,48*(20*365+5)), lwdown_h(1,1,48*(20*365+5))
real rainf_h(1,1,48*(20*365+5)), snowf_h(1,1,48*(20*365+5))

real tair(1,1,24*(20*365+5)), rh(1,1,24*(20*365+5)), wind(1,1,24*(20*365+5))
real swdown(1,1,24*(20*365+5)), lwdown(1,1,24*(20*365+5))
real rainf(1,1,24*(20*365+5)), snowf(1,1,24*(20*365+5)) 

integer psurf_h(1,1,48*(20*365+5)), psurf(1,1,24*(20*365+5)), gmt_offset

real zbot(1,1,24*(20*365+5))
...
site = '1x1pt_AU-EuF'
data_fname = (/ 'EucFACE_forcing_1992-2011', &
...
do i=1,i_steps
 read(50,*) year, doy, hod, swdown_h(1,1,i), trash, lwdown_h(1,1,i), &
	...
	rh_h(1,1,i),  wind_h(1,1,i), psurf_h(1,1,i), trash, trash   
enddo

else
!open CDF data file
data_path = './EucFACE_met_data/'//trim(data_fname(md))//trim(fend1(md))//trim(fend2(2))//trim(fend3(2))
...
RCODE = NF90_INQ_VARID(NCID,'longitude',varid)
RCODE = NF90_GET_VAR(NCID, varid, lon)
RCODE = NF90_INQ_VARID(NCID,'latitude',varid)
RCODE = NF90_GET_VAR(NCID, varid, lat)
RCODE = NF90_INQ_VARID(NCID,'Tair',varid)
RCODE = NF90_GET_VAR(NCID, varid, tair_h,start=(/ 1,1,1 /),count=(/ 1,1,i_steps /))
!print*, 'read Tair ', data_path
!print*, nf90_strerror(RCODE)

RCODE = NF90_INQ_VARID(NCID,'RH',varid)
RCODE = NF90_GET_VAR(NCID, varid, rh_h,start=(/ 1,1,1 /),count=(/ 1,1,i_steps /))
RCODE = NF90_INQ_VARID(NCID,'Wind',varid)
RCODE = NF90_GET_VAR(NCID, varid, wind_h,start=(/ 1,1,1 /),count=(/ 1,1,i_steps /))
RCODE = NF90_INQ_VARID(NCID,'SWdown',varid)
RCODE = NF90_GET_VAR(NCID, varid, swdown_h,start=(/ 1,1,1 /),count=(/ 1,1,i_steps /))
RCODE = NF90_INQ_VARID(NCID,'LWdown',varid)
RCODE = NF90_GET_VAR(NCID, varid, lwdown_h,start=(/ 1,1,1 /),count=(/ 1,1,i_steps /))
RCODE = NF90_INQ_VARID(NCID,'PSurf',varid)
RCODE = NF90_GET_VAR(NCID, varid, psurf_h,start=(/ 1,1,1 /),count=(/ 1,1,i_steps /))
if(verbose) print*, nf90_strerror(RCODE), psurf_h(1,1,1)

RCODE = NF90_INQ_VARID(NCID,'Rainf',varid)
RCODE = NF90_GET_VAR(NCID, varid, rainf_h,start=(/ 1,1,1 /),count=(/ 1,1,i_steps /))
...
if(gmt_offset.ne.0) then
 ...
 psurf(1,1,1:(i_steps-gmt_offset)) = psurf(1,1,(gmt_offset+1):i_steps)
 psurf(1,1,(i_steps-gmt_offset+1):i_steps) = psurf(1,1,1:gmt_offset)
 ...
 psurf_h(1,1,1:(i_steps-gmt_offset)) = psurf_h(1,1,(gmt_offset+1):i_steps)
 psurf_h(1,1,(i_steps-gmt_offset+1):i_steps) = psurf_h(1,1,1:gmt_offset)
...
!    RCODE = NF90_PUT_ATT(NCID, varid, 'units', 'K')
    ...
    RCODE = NF90_PUT_VAR(NCID, varid, psurf(1,1,ms:me),start=(/ 1,1,1 /),count=(/ 1,1,dimlength /))	
...
!    RCODE = NF90_PUT_ATT(NCID, varid, 'units', 'K')
    ...
    RCODE = NF90_INQ_VARID(NCID,'PSRF',varid)
    RCODE = NF90_PUT_VAR(NCID, varid, psurf_h(1,1,ms:me),start=(/ 1,1,1 /),count=(/ 1,1,dimlength /))	
!    RCODE = NF90_PUT_ATT(NCID, varid, 'long_name', 'pressure at the lowest atm level (TBOT)')
!    RCODE = NF90_PUT_ATT(NCID, varid, 'units', 'Pa')
    if(verbose) print*, 'write PSRF ', psurf_h(1,1,ms)
    if(verbose) print*, nf90_strerror(RCODE)
